...
这个版本修改自：https://ai.ucas.ac.cn/index.php/zh-cn/jxjy/jyyssd1/7471-latex。

一些重要修改说明如下：

* 首先修复了英文摘要超过两页后，出现的偶数页页眉出现中文论文标题的问题。
* 增加了一键切换送审/正式版的功能。方便用于送审和最后存档提交。

一键切换送审/正式版的功能说明如下，在 `Thesis.tex` 中，修改以下几行，

```latex
\newif\ifblind
%\blindtrue  % 双盲版本 （送审版）
\blindfalse % 非双盲版本（正式版）
```

送审版注释第三行，正式版注释第二行。比如下面是送审版的格式：
...
注意：**在切换版本的时候，需要多编译一次，否则目录更新会有延迟。**

送审版默认隐去作者、导师、成果、致谢等信息。如果需要隐去更多的信息，使用方法可以参考 `Tex/Backmatter.tex` 中的写法，即
